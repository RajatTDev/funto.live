...
const { createVersionIfNeeded, getLatestVersion } = require("../../util/versionUtils");
...
exports.updateLanguageTranslations = async (req, res) => {
    try {
        ...
        if (changeLogs.length) {
            ...
            await Translation.updateOne(
                { languageCode: languageCode?.trim()?.toLowerCase(), module: module?.trim()?.toLowerCase() },
...
exports.getLanguage = async (req, res) => {
    try {
        ...
        if (!module || !module.trim() || !["app", "web"].includes(module?.trim()?.toLowerCase())) {
            return res.status(200).json({ status: false, message: "Invalid module, please provide valid module(app/web)" })
        }

        const doc = await Translation.findOne({ languageCode: languageCode?.trim()?.toLowerCase(), module: module?.trim()?.toLowerCase() }).lean();
